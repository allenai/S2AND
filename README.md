...
- `production_model_v1.2.pickle` is the current recommended model.
- Models `v1.1` and `v1.2` were trained with `compute_reference_features=False`.
...
Minimal input shape for `v1.1` and `v1.2`:
...
SPECTER embeddings can be sourced from the Semantic Scholar API. Use `embedding.specter_v2` with `v1.2` and `embedding.specter_v1` with `v1.1`.
...
- `predict(..., batching_threshold=...)` and `predict_incremental(..., batching_threshold=...)` use subblocking to keep work bounded.
- `predict_incremental(...)` returns `phase_b_mode`, which is:
  - `exact` when Phase B ran globally.
  - `subblock_local` when runtime fell back to approximate per-subblock behavior to stay within budget.
- `total_ram_bytes` and `max_chunk_pairs` are the main memory-control knobs for large inference jobs.
