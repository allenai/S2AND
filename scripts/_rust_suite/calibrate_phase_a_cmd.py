...
def _open_text_log(path: Path):
    # PowerShell redirection (`*> file.log`) often writes UTF-16LE with a BOM.
    # We want calibration to work on those logs without requiring manual re-encoding.
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Calibrate Phase A accumulator entry bytes from telemetry logs.")
    parser.add_argument("logs", nargs="+", help="Log file(s) containing 'Telemetry: phase_split_phase_a' lines.")
    parser.add_argument(
        "--write-json",
        type=str,
        default=None,
    ...
    for raw_path in args.logs:
        ...
        with _open_text_log(path) as fh:
            for record in iter_log_records(fh):
                if "Telemetry: phase_split_phase_a" not in record:
                    continue
                phase_a_records += 1
    ...
    if not summary:
        print(f"No calibration samples found (matched Phase A records={phase_a_records}, effective_samples=0).")
        return 2

    recommended_p95 = int(math.ceil(summary["p95"]))
    print(
        "Phase A accumulator calibration:\n"
        f"- matched_phase_a_records={phase_a_records}\n"
        f"- effective_samples={len(effective_values)}\n"
        f"- bytes_per_entry: min={summary['min']:.1f} mean={summary['mean']:.1f} "
        f"p50={summary['p50']:.1f} p95={summary['p95']:.1f} max={summary['max']:.1f}\n"
        f"- recommended_accumulator_entry_bytes_p95={recommended_p95}"
    )
    ...
    if args.write_json is not None:
        ...
        blob = {
            ...
            "python": sys.version,
            "matched_phase_a_records": int(phase_a_records),
            "effective_samples": int(len(effective_values)),
            "recommended_accumulator_entry_bytes_p95": int(recommended_p95),
