...
| Name alias data | Some paths could pass per-dataset Arrow `name_pairs` / `name_tuples` overrides. | Python validates the shared artifact once and passes explicit pairs to Rust; production path bundles must not carry alias override paths. |
...
| Raw incremental candidate planning | Explicit requests use `RawBlockQueryCandidatePlanner.from_query_signatures(paths_with_query_signatures_and_batch_indexes, ...)`; automatically selected promoted query batches use `from_auto_queries(paths_with_batch_indexes, ...)`. Both reuse the planner across `.plan(...)` calls. | Unindexed filtered Arrow scans, Python mini object materialization, temporary empty request sidecars, direct retriever wiring from callers. |
