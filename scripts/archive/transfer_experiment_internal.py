...
"""
Historical full internal transfer workload used as the behavioral authority for
``scripts/_rust_suite/transfer_mini_cmd.py``.

Warning:
    This workflow requires private datasets and its historical ``plot_facets``
    call no longer matches the canonical-v2 plotting API. Retain it as an
    archival workload specification unless that call is migrated separately.
