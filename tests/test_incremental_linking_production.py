...
def test_query_disallow_replay_redecides_without_retrieval_or_featurization() -> None:
    candidate_batch = LinkerCandidateBatch(
        ...
        row_component_keys=("shared", "runner-up"),
        retrieval_ranks=np.asarray([1, 2], dtype=np.uint16),
    )
    feature_matrix = LinkerFeatureMatrix(
        matrix=np.zeros((2, len(promoted_linker_feature_columns())), dtype=np.float32),
        feature_columns=promoted_linker_feature_columns(),
        candidate_batch=candidate_batch,
    )
    result = SimpleNamespace(
        feature_matrix=feature_matrix,
        compact_result=SimpleNamespace(probabilities=np.asarray([0.95, 0.70], dtype=np.float64)),
        decision_row_signals={"first_name_bucket": np.asarray(["multi_letter_first"] * 2, dtype=object)},
    )
    replay = runtime_module._build_query_decision_replay(  # noqa: SLF001
        result,
        query_signature_index=10,
    )
    ...
    class Artifact:
        ...
        def predict_probabilities(_matrix, *, num_threads=None):
            del num_threads
            return np.asarray([0.95, 0.70], dtype=np.float64)

    decision, telemetry = runtime_module._redecide_query_from_replay(  # noqa: SLF001
        Artifact(),
        replay,
        excluded_components={"shared"},
    )

    assert decision.action == "link"
    assert decision.component_key == "runner-up"
    assert replay.estimated_bytes > 0
    assert telemetry["cluster_seed_disallow_excluded_row_count"] == 1


def test_query_replay_row_index_is_built_once_and_rejects_cross_query_rows() -> None:
    candidate_batch = LinkerCandidateBatch(
        row_count=4,
        left_signature_indices=np.zeros(0, dtype=np.uint32),
        right_signature_indices=np.zeros(0, dtype=np.uint32),
    ...
    result = SimpleNamespace(
        feature_matrix=feature_matrix,
        compact_result=SimpleNamespace(probabilities=np.asarray([0.9, 0.8, 0.7, 0.6], dtype=np.float64)),
        decision_row_signals={},
    )

    rows_by_query = runtime_module._query_row_indices_by_query_index(candidate_batch)  # noqa: SLF001
    replay = runtime_module._build_query_decision_replay(  # noqa: SLF001
        result,
        query_signature_index=11,
        row_indices=rows_by_query[11],
    )

    assert rows_by_query[10].tolist() == [0, 1]
    assert rows_by_query[11].tolist() == [2, 3]
    assert replay.feature_matrix.candidate_batch.row_component_keys == ("c", "d")
    try:
        runtime_module._build_query_decision_replay(  # noqa: SLF001
            result,
            query_signature_index=11,
            row_indices=rows_by_query[10],
        )
    except ValueError as error:
        assert "different query" in str(error)
    else:
        raise AssertionError("cross-query replay rows must be rejected")
