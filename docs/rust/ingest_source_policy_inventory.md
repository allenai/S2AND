...
| Name counts | Uses `name_counts_index` when present; rejects a `name_counts.arrow` path without the index. | Opens the same validated shared index, performs bounded native batch lookup during preprocessing, then reads the attached four-scalar `author_info_name_counts`. |
