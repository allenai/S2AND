"""Packaged FeatureBlock conversion helpers for writing Arrow IPC tables from ANDData.

These helpers let a service (e.g. the author-disambiguation service) convert an
in-memory ``ANDData`` into the Arrow IPC artifacts that
``Clusterer.predict_from_arrow_paths`` consumes, without depending on the
unpackaged ``scripts/`` tree. The conversion logic previously lived in
``scripts/arrow_conversion_helpers.py``, which now re-exports from here.
"""
