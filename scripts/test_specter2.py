...
datasets = ["arnetminer", "inspire", "kisti", "pubmed", "qian", "zbmath", "inventors_s2and"]
...
model_path = os.path.join(PROJECT_ROOT_PATH, "data", MODEL)
    ...
    for dataset_name in datasets:
        ...
        if TRAIN_FLAG:
            ...
            clusterer = Clusterer(
                ...
                nameless_featurizer_info=nameless_featurization_info,
                random_state=random_seed,
                # This is an option used by the S2 production system but not in the S2AND paper.
...
clusterer1 = load_pickle_with_verified_label_encoder_compat("data/production_model_v1.1.pickle")["clusterer"]
...
for dataset_name in ["aminer", "arnetminer", "kisti", "qian", "zbmath", "inspire"]:
    ...
    (4) use clusterer2.predict_incremental on the test using anddata2 vs
        use clusterer1.predict_incremental on the test using anddata2
    ...
    for _ in range(1000):
        ...
        if len(random_block_test) > 0 and len(random_block_train) > 0:
            # (2) use clusterer1.predict for the train using anddata1
            # For this we need to format the block signatures into the correct format for predict.
            ...
            # (3) populate anddata1.cluster_seeds_require and anddata2.cluster_seeds_require with the results of (2)
            for i, (_cluster_id, cluster) in enumerate(train_cluster_prediction.items()):
            ...
            # (4) use clusterer2.predict_incremental on the test using anddata2 vs
            # use clusterer1.predict_incremental on the test using anddata2
            test_preds_1 = clusterer1.predict_incremental(random_block_test, anddata1)["clusters"]
            test_preds_2 = clusterer2.predict_incremental(random_block_test, anddata2)["clusters"]
            ...
            def add_missed_ones_to_test_preds(train_cluster_prediction, test_preds):
                ...
                # For each list in train_cluster_prediction.values() that is not in all_the_values_in_test_preds,
                # add it to test_preds_1.
